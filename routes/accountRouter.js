...
accountRouter.post("/register", async (req, res) => {
  try {
    ...
    await db.query(
      "INSERT INTO users(first_name, last_name, password_hash, email, address) VALUES ($1, $2, $3, $4, $5)",
      [first_name, last_name, hashedPass, email, address]
    );
    res.status(201).send();
  } catch (err) {
    console.error(err);
  ...
  if (req.user) {
    try {
      ...
      const result = await db.query(
        "UPDATE users SET (first_name, last_name, address) = ($2, $3, $4) WHERE user_id = $1 RETURNING *",
